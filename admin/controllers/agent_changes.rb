AutoAftermarketApi::Admin.controllers :agent_changes do
  ...
  post :change do
    begin
      ...
      raise "专属客服不能为空" if params[:new_agent_id].blank?
      @agent_change.update!(old_agent_id: @agent_change.account.info_service_id, staff_id: current_account.id, status: 1)
      @agent_change.account.update!(info_service_id: params[:new_agent_id])
