...
=end
  post :to_dos, :provides => [:json] do
    api_rescue do
      ...
      @order_skus = OrderSku.where(comment_status: 0).where("(SELECT orders.account_id FROM orders WHERE orders.status = 'done' and orders.order_no = order_skus.order_no) = #{@customer.id}")
