...
=end
  post "/", :provides => [:json] do
    api_rescue do
      ...
      { status: 'succ', data: @merchant.coupons.map(&:to_api)}.to_json
    end
  end
    ...
    api_rescue do
      ActiveRecord::Base.transaction do
        ...
        @request_params['coupon_ids'].each do |coupon_id|
          ...
          CouponReceive.create!(customer_id: @customer.id, coupon_id: @coupon.id, coupon_money: @coupon.money, full_money: @coupon.full_money, status: 0)
        end
      end
